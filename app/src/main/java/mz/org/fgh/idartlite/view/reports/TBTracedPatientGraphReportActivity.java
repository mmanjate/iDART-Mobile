...
import mz.org.fgh.idartlite.viewmodel.report.TBTracedPatientsGraphReportVM;

public class TBTracedPatientGraphReportActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tb_traced_patient_graph_report);
        ...
        edtStart.setOnClickListener(view -> {
            ...
            DatePickerDialog datePickerDialog = new DatePickerDialog(TBTracedPatientGraphReportActivity.this, (view1, year, monthOfYear, dayOfMonth) -> {
        ...
        /*    if (getRelatedViewModel().isReportGenerationOnProgress()){
            ...
            if (!Utilities.stringHasValue(start) || !Utilities.stringHasValue(end) ){
                Utilities.displayAlertDialog(TBTracedPatientGraphReportActivity.this, "Por favor indicar o período por analisar!").show();
            }else
            if (DateUtilities.dateDiff(DateUtilities.createDate(end, DateUtilities.DATE_FORMAT), DateUtilities.createDate(start, DateUtilities.DATE_FORMAT), DateUtilities.DAY_FORMAT) < 0){
                Utilities.displayAlertDialog(TBTracedPatientGraphReportActivity.this, "A data inicio deve ser menor que a data fim.").show();
            }else
            if ((int) (DateUtilities.dateDiff(DateUtilities.getCurrentDate(), DateUtilities.createDate(start, DateUtilities.DATE_FORMAT), DateUtilities.DAY_FORMAT)) < 0){
                Utilities.displayAlertDialog(TBTracedPatientGraphReportActivity.this, "A data inicio deve ser menor que a data corrente.").show();
            }
            else
            if ((int) DateUtilities.dateDiff(DateUtilities.getCurrentDate(), DateUtilities.createDate(end, DateUtilities.DATE_FORMAT), DateUtilities.DAY_FORMAT) < 0){
                Utilities.displayAlertDialog(TBTracedPatientGraphReportActivity.this, "A data fim deve ser menor que a data corrente.").show();
            }else {
    ...
    public void generateGraph(List<ClinicInformation> clinicInformations) {
        ...
        cdSuspeito.setName("Pacientes Suspeitos com TB");
        Number[] suspectData = new Number[categories.size()];


        int countTotal;
        int countTbSuspeitos;
        ...
        for (String us : categories){
            ...
            for (ClinicInformation clinicInformation : clinicInformations){
                if (clinicInformation.getPatient().getEpisodes1().iterator().next().getSanitaryUnit().equals(us)){
                    ...
                    if ( clinicInformation.getTbSuspectReport()) countTbSuspeitos ++;
            ...
            suspectData[i] = countTbSuspeitos;
    ...
    @Override
    public TBTracedPatientsGraphReportVM getRelatedViewModel() {
        return (TBTracedPatientsGraphReportVM) super.getRelatedViewModel();
    }

    @Override
    public BaseViewModel initViewModel() {
        return new ViewModelProvider(this).get(TBTracedPatientsGraphReportVM.class);
    }
